...
TNX_AGE_IDLE_TNX = "SELECT EXTRACT (EPOCH FROM MAX(age(NOW(), query_start))) as d FROM pg_stat_activity WHERE state='<IDLE> in transaction'"
TNX_AGE_RUNNING_ALL = "SELECT EXTRACT (EPOCH FROM MAX(age(NOW(), query_start))) as d FROM pg_stat_activity WHERE state<>'<IDLE> in transaction' AND state<>'<IDLE>'"
TNX_AGE_RUNNING = """
SELECT
